...
export default function Profile() {
  ...
  const changePassword = async () => {
    ...
    try {
      ...
      setPwMsg(e.response?.data?.detail || 'Errore');
    }
  };

  const deleteAccount = async () => {
    if (!window.confirm('Sei sicuro? Questa azione e irreversibile. Digita ELIMINA per confermare.')) return;
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-3xl font-bold gradient-text mb-2" data-testid="profile-title">Il tuo Profilo</h1>
      <p className="text-[var(--text-secondary)] mb-8 text-sm">Gestisci le impostazioni del tuo account.</p>

      {/* Profile Info */}
      <div className="card mb-6">
        <h3 className="font-semibold mb-4 flex items-center gap-2"><User size={20} /> Dati Personali</h3>
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">Settore principale</label>
          ...
          <button data-testid="profile-save-btn" className="btn-gradient" onClick={saveProfile}>
            {saved ? <><Check size={16} /> Salvato!</> : 'Salva Modifiche'}
          </button>
        </div>
      </div>
      ...
      {/* Plan */}
      <div className="card mb-6">
        <h3 className="font-semibold mb-2">Piano Abbonamento</h3>
      ...
      {/* Change Password */}
      <div className="card mb-6">
        <h3 className="font-semibold mb-4 flex items-center gap-2"><Lock size={20} /> Cambio Password</h3>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">Password attuale</label>
            <input type="password" className="input-dark" value={currentPw} onChange={e => setCurrentPw(e.target.value)} style={{ paddingLeft: '1rem' }} />
          </div>
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">Nuova password</label>
            <input type="password" className="input-dark" value={newPw} onChange={e => setNewPw(e.target.value)} placeholder="Min 8 caratteri" style={{ paddingLeft: '1rem' }} />
          </div>
          {pwMsg && <p className="text-sm" style={{ color: pwMsg.includes('successo') ? 'var(--accent-green)' : 'var(--accent-pink)' }}>{pwMsg}</p>}
          <button className="btn-ghost" onClick={changePassword}>Cambia Password</button>
        </div>
      </div>

      {/* Delete Account */}
      <div className="card" style={{ borderColor: 'rgba(236,72,153,0.3)' }}>
        <h3 className="font-semibold mb-2 flex items-center gap-2 text-[var(--accent-pink)]"><Warning size={20} /> Zona Pericolosa</h3>
