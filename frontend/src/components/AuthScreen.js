...
import { EnvelopeSimple, Lock, ArrowRight, User } from '@phosphor-icons/react';
...
export default function AuthScreen() {
  ...
  useEffect(() => {
    ...
    const billing = params.get('billing') === 'success';
    setBillingSuccess(billing);
    if (billing) setMode('register');
    if (rt) {
      setResetToken(rt);
      setShowForgot(true);
    }
  ...
  const lockRegistrationFlow = billingSuccess;
  ...
  const handleReset = async () => {
    setResetMsg('');
    try {
      const { data } = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/auth/reset-password`, {
        token: resetToken,
      ...
      setResetMsg(e.response?.data?.detail || 'Errore. Riprova.');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(typeof detail === 'string' ? detail : t(isRegisterMode ? 'auth.registerError' : 'auth.loginError'));
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        <div className="mb-8 text-center">
          ...
          <p className="text-[var(--text-secondary)] text-sm">
            {billingSuccess
              ? 'Pagamento completato. Crea ora il tuo account per accedere a Sketchario.'
              : isRegisterMode
          ...
          {!isRegisterMode && (
            <p className="text-center mt-3">
              <button
                ...
                className="text-sm text-[var(--text-muted)] hover:text-[var(--gradient-start)] underline"
